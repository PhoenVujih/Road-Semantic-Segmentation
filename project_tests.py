...
def test_optimize(optimize):
    ...
    logits, train_op, total_loss, classification_loss = optimize(layers_output, correct_label, learning_rate, num_classes)
    ...
    with tf.Session() as sess:
        ...
        test, loss1, loss2 = sess.run([layers_output, total_loss, classification_loss], {correct_label: np.arange(np.prod(shape)).reshape(shape)})
...
def test_train_nn(train_nn):
    ...
    classification_loss = tf.constant(10.11)
    total_loss = tf.constant(10.99)
